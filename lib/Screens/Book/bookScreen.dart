...
class BookScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    LayoutCubit.get(context).getbooks();
    NotebookModel bookmodel;
      ...
      builder: (context, state) {
        return Scaffold(
          appBar: AppBar(
            ...
            elevation: 0,
            title: Center(child: Text("المذكرات")),
          ),
          body: SafeArea(
              child: Center(
            child: Column(children: [
              ...
              if (state is BookLoadingState) LinearProgressIndicator(),
              SizedBox(
                height: 10,
              ),
              if (state is BookSuccessState)
                Expanded(
                    child: ListView.separated(
                        itemBuilder: (context, index) => GestureDetector(
                              onTap: () {
                                navigateTo(
                                    context,
                                    bookDetail(
                                        notebook: LayoutCubit.get(context)
                                            .bookmodel
                                            .data![index]));
                              },
                              child: buildbookItem(
                        ...
                        itemCount:
                            LayoutCubit.get(context).bookmodel.data!.length))
            ]),
          )),
        );
      },
  ...
  Widget buildbookItem(noteData bmodel, context) => Padding(
